...
def yahoo_quote_batch(symbols: List[str]) -> Dict[str, Dict[str, Any]]:
    ...
    if need:
        ...
        for i in range(0, len(need), 50):  # Yahoo 一次建議 <=50
            batch = need[i:i+50]
            try:
...
def get_options_slice(symbol: str, expiry: Optional[str]) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        # 補價
        need_syms = [s for s, row in zip(call_occs+put_occs, call_rows+put_rows)
                     if row["bid"] is None or row["ask"] is None or row["last"] is None]
        if need_syms:
            quotes_map = yahoo_quote_batch(need_syms)
            def apply_fill(rows: List[Dict[str, Any]], occs: List[str]):
                for idx, occ in enumerate(occs):
                    ...
                    if bid is not None and ask is not None:
                        rows[idx]["mid"] = round((bid + ask) / 2, 2); rows[idx]["mid_from"] = "ba"
                    elif last is not None:
                        rows[idx]["mid"] = round(last, 2); rows[idx]["mid_from"] = "last"
            apply_fill(call_rows, call_occs)
            apply_fill(put_rows, put_occs)

        block = {"symbol": symbol.upper(), "expiry": picked, "time": _now_iso(),
